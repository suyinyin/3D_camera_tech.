...
int main(int argc, char const* argv[])
{
    ...
    //std::vector<Balltracking> ball_color = {orange};
    ...
    while (cv::waitKey(30) != 27)
    {
        ...
        // with a threshold segmentation and a Hough transform
            ...
            if (circles.size() <= 3 && circles.size() != 0) {
                ...
                ele.drawHoughCircles(frames[index], detect_circle);
            }
            ...
            if (ball_found_prev[index])
                ele.drawTrackingInfo_roi(frames[index], positions[index]);
            else
                ele.drawTrackingInfo_trajectory(frames[index], positions[index]);
            
            ball_found_prev[index] = ball_found[index];
            
            frame_to_show.push_back(frames[index]);
            index++;
        }
        ...
        if (ball_position[0].x == 0 && ball_position[0].y == 0 &&
            ball_position[1].x == 0 && ball_position[1].y == 0 &&
            ball_position[2].x == 0 && ball_position[2].y == 0 &&
            ball_position[3].x == 0 && ball_position[3].y == 0)  continue;


        output_position_uv << time_stamp << "\t";
        for (auto& ball_pos : ball_position) {
            // store 3d coordinate value of all frame. Units: milimeter 
            std::vector<int> point_3d_value = { 0,0,0 };
            if (ball_pos.x == 0 || ball_pos.y == 0) {
                output_position_uv << setw(7) << point_3d_value[0] << setw(7) << point_3d_value[1] << setw(7) << point_3d_value[2];
                continue;
            }
            unsigned short depth_value = 0;
            my_realsenses.get_points_cloud(file_index, depth_value, ball_pos.x, ball_pos.y, 0);
            if (depth_value == 0) { 
                output_position_uv << setw(7) << point_3d_value[0] << setw(7) << point_3d_value[1] << setw(7) << point_3d_value[2];
                continue; }

            my_realsenses.get_point_3Dvalue(point_3d_value, ball_pos.x, ball_pos.y);
            output_position_uv << setw(7) << point_3d_value[0] << setw(7) << point_3d_value[1] << setw(7) << point_3d_value[2];

        }
        output_position_uv << std::endl;
        file_index++;        
    }
