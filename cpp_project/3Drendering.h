...
public:
    ...
    window(int width, int height, const char* title, int position_u, int position_v)
        : _width(width), _height(height)
    {
        ...
        // set window position
        glfwSetWindowPos(win, position_u, position_v);
...
void render_scene(glfw_state app_state)
{
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluPerspective(60.0, 4.0 / 3.0, 1, 60);
...
void  draw_axes(float axis_size = 1.f, float axisWidth = 4.f)
{
    // Triangles For X axis
    glBegin(GL_TRIANGLES);
    glColor3f(1.0f, 0.0f, 0.0f);
...
// draw trajectory
void  draw_trajectory(std::vector<rs2_vector>& trajectory) {
